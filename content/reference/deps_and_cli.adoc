...
[source]
----
clj     [clj-opt*] [execution-opt]
clojure [clj-opt*] [execution-opt]
----

== Start a REPL

To run a REPL, invoke `clj` without any execution-opt:
...
Use the following execution-opt to invoke a function that takes a map:
...
[NOTE]
====
Alias arg maps can also include a default namespace or namespace aliases for resolving the function (see "Function execution" for more details).
====

== Running a main or script

You can use the `-M` execution-opt to invoke clojure.main, which supports calling a namespace with a `-main` function or a Clojure script:
...
[source,clojure]
----
clj -X clojure.core.server/start-server :name '"repl"' :port 5555 :accept clojure.core.server/repl
----
...
[source,clojure]
----
(clojure.core.server/start-server {:name "repl" :port 5555 :accept clojure.core.server/repl})
----

You could also set this up as a default namespace in your `deps.edn`:
...
{:aliases
 {:server
  {:ns-default clojure.core.server}}}

clj -X:server start-server :name '"repl"' :port 5555 :accept clojure.core.server/repl
----

Or you could just make an alias (along with others):
...
{:aliases
 {:project
  {:ns-aliases {server clojure.core.server}}}}

clj -X:project server/start-server :name '"repl"' :port 5555 :accept clojure.core.server/repl
----
...
Note that strings require both single quotes for the shell and double quotes to be properly read as edn!
